...
- If the forwarded request includes `--sandbox`, pass it through to `task` before the task text and do not add `--write`.
...
- Never add `--sandbox danger-full-access` unless the user explicitly asked for it. It disables the Codex sandbox entirely.
