...
- If the user passes `--sandbox <read-only|workspace-write|danger-full-access>` before the task text, forward it to `task` in that position and do not add `--write`. The sandbox flag already decides whether Codex may edit. A `--sandbox` inside the task text is part of the prompt: leave it there, and do not move it in front.
